...
class TransNet(nn.Module):
    def __init__(self, hidden_dim):
        ...
        self.split_weight = nn.Parameter(torch.eye(16))
    ...
    def forward(self, silho, batch_frame=None):
        ...
        for num_bin in self.bin_num:
            ...
            global_feature.append(z)
        global_feature = torch.cat(global_feature, 2).permute(2, 0, 1).contiguous()
        global_feature = global_feature.matmul(self.fc_bin)

        x = x.mean(-1) + x.max(-1)[0]
        weight = self.split_weight.masked_fill(self.split_weight == 0, -1e9)
        weight = F.softmax(weight, dim=0)
        x = torch.einsum('nsct, tt-> nsct', x, weight)
        x = x.permute(3, 0, 1, 2).contiguous()
        local_feature = self.transformer(x, None)
        local_feature = local_feature.max(2)[0]
        feature = torch.cat((global_feature, local_feature), 0)
        feature = feature.transpose(0, 1).contiguous()
