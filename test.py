...
parser.add_argument('--iter', default='120000', type=int,
...
print('Evaluating...')
acc, test_loss = evaluation(test, conf['data'])
...
for i in range(1):
    ...
    print('NM:', average(acc[0, :, :, i]))
    print('BG:', average(acc[1, :, :, i]))
    print('CL:', average(acc[2, :, :, i]))

print('===Min_distance (Include identical-view cases)===')
print('NM: %.3f,\tBG: %.3f,\tCL: %.3f' % (
        np.mean(test_loss[0, :, :, :]),
        np.mean(test_loss[1, :, :, :]),
        np.mean(test_loss[2, :, :, :])))'''
